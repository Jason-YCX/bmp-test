...
export default {
  ...
  created() {
    ...
    const localPageNum = (this.pageURI && currentPage && parseInt(currentPage)) || this.currentPage
  ...
  methods: {
    ...
    loadData(pagination, filters, sorter) {
      ...
      const request = this.data(parameter)
      // 对接自己的通用数据接口需要修改下方代码中的 r.currentPage, r.totalCount, r.data
      // eslint-disable-next-line
      if ((typeof request === 'object' || typeof request === 'function') && typeof request.then === 'function') {
        request
          .then(r => {
            const [ok, result] = this.$requestOk(r)
            if (!ok) return
            if (Array.isArray(result)) {
              this.localDataSource = result
              this.localPagination = false
            } else {
              const { currentPage, list, totalCount } = result
              this.localPagination =
                (this.showPagination &&
                  Object.assign({}, this.localPagination, {
                    current: currentPage, // 返回结果中的当前分页数
              ...
              //   if (
              //     ['auto', true].includes(this.showPagination) &&
              //     totalCount <= currentPage * this.localPagination.pageSize
              //   ) {
